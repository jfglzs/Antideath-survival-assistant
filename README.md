...
### 整理背包杂物
按下快捷键后开始整理背包，把所有背包内的物品（包括潜影盒内的物品），全部集中到背包内排序靠前的潜影盒里，以腾出空潜影盒。

需要安装 **快捷潜影盒 (Quick Shulker)**。
...
### 整理背包杂物-白名单
配置整理白名单，仅整理列表中的物品（填物品 ID，如 `minecraft:dirt`）。
- 类型：`字符串列表`
- 默认值：`[]`
- 分类：`ALL`，`ORGANIZE`
...
### 整理背包杂物-黑名单
配置整理黑名单，不整理列表中的物品（填物品 ID，如 `minecraft:stone`）。
- 类型：`字符串列表`
- 默认值：`[]`
- 分类：`ALL`，`ORGANIZE`