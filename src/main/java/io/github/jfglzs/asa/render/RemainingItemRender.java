    ...
    public void onRenderGameOverlayPost(GuiGraphics drawContext) {
        if (Configs.DISPLAY_REMAIN_ITEM.getBooleanValue()) {
            ...
            if (stack != null && !stack.getItem().equals(Items.AIR)) {
                drawContext.drawString(Minecraft.getInstance().font, "%s %s".formatted(stack.getHoverName().getString(), PlayerUtils.checkRemainCount(stack.getItem())), xOffset + 20, yOffset + 4, 0xFFFFFFFF,true);
                drawContext.renderItem(stack, xOffset, yOffset);
            }
        }
    }
}
